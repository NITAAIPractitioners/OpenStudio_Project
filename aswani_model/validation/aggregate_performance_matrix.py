...
for _m, _cfg in MODELS.items():
    ...
    print(f"Loaded: {_m} -> latest run: {_cfg['run'] or '(none found)'}")
...
def get_metrics_for_run(model_name, run_id):
    ...
    # Track metrics for ALL zones to satisfy the "apply for all zones" requirement
    TRACKB_ZONES = ['TZ_NW', 'TZ_S', 'TZ_E', 'TZ_W', 'TZ_C']
    results = {}
    
    # Pre-load all CSV data into memory to avoid repeated reads
    metric_dfs = {}
    for var in VARIABLES:
        csv_path = VALIDATION_DIR / "outputs" / model_name / var / f"{var}_metrics_summary.csv"
        if csv_path.exists():
            metric_dfs[var] = pd.read_csv(csv_path)

    for zone in TRACKB_ZONES:
        # Occupancy
        os.environ["VAL_SQL_PATH"] = str(sql_path)
        # Note: sv.load_simulation_variable is NW focused in its default logic?
        # No, it takes zone as argument. Let's ensure it works for others too.
        series = sv.load_simulation_variable("Zone People Occupant Count", zone)
        avg_occ = series.mean() if series is not None else 0
        results[f"Avg_Sim_Occ_{zone}"] = avg_occ
        
        for var in VARIABLES:
            if var in metric_dfs:
                df = metric_dfs[var]
                col_name = 'RMSE_C' if var=='temperature' else 'RMSE_RH' if var=='humidity' else 'RMSE_CO2'
                mae_col  = 'MAE_C'  if var=='temperature' else 'MAE_RH'  if var=='humidity' else 'MAE_CO2'
                
                z_metrics = df[df['Zone'] == zone]
                results[f"{var}_RMSE_{zone}"] = z_metrics[col_name].values[0] if not z_metrics.empty else 0
                results[f"{var}_MAE_{zone}"]  = z_metrics[mae_col].values[0]  if not z_metrics.empty else 0
            else:
                results[f"{var}_RMSE_{zone}"] = 0
                results[f"{var}_MAE_{zone}"]  = 0

    # Calculate Building-Wide Mean RMSE and MAE for each variable
    for var in VARIABLES:
        zone_rmses = [results[f"{var}_RMSE_{z}"] for z in TRACKB_ZONES if results.get(f"{var}_RMSE_{z}", 0) > 0]
        results[f"Mean_{var}_RMSE"] = sum(zone_rmses) / len(zone_rmses) if zone_rmses else 0
        
        zone_maes = [results[f"{var}_MAE_{z}"] for z in TRACKB_ZONES if results.get(f"{var}_MAE_{z}", 0) > 0]
        results[f"Mean_{var}_MAE"] = sum(zone_maes) / len(zone_maes) if zone_maes else 0

    # Maintain NW specific naming for the "Golden Zone" primary tracking
    results["Avg_Sim_Occ_NW_Primary"] = results["Avg_Sim_Occ_TZ_NW"]
    results["temperature_RMSE_NW_Primary"] = results["temperature_RMSE_TZ_NW"]
    results["temperature_MAE_NW_Primary"] = results["temperature_MAE_TZ_NW"]
    results["co2_RMSE_NW_Primary"] = results["co2_RMSE_TZ_NW"]
    results["co2_MAE_NW_Primary"] = results["co2_MAE_TZ_NW"]
