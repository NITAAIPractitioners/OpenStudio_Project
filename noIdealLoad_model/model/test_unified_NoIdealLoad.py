...
parser = argparse.ArgumentParser(description='NoIdealLoad Model Sensitivity Run.')
...
for sname in space_map.keys():
    ...
    if f_p.exists() and not args.use_baseline:
        ...
        for d in df['dt'].dt.date.unique():
            ...
            if args.eq_sch_type == 'Scaled': lg_val = max(0.3, last_val) if last_val >= 0.1 else 0.2
            elif args.eq_sch_type == 'Constant': lg_val = 1.0
            elif args.eq_sch_type == 'Softer': lg_val = max(0.15, last_val) if last_val >= 0.1 else 0.1
            else: lg_val = max(0.3, last_val) if last_val >= 0.1 else 0.2
...
# Output variables for NoIdealLoad (free-floating) baseline.
# IdealLoads-specific variables are intentionally excluded.
output_vars = [
