"""
validate_temperature.py — idealLoad_model
=========================================
...
def validate_zone(zone, offices, sql_path):
    ...
    # Load simulated (override global SQL_PATH if needed or use environment if shared_validation allows)
    # Since load_simulation_variable uses SQL_PATH from env/glob, we set it here
    ...
    # Plot
    plot_path = OUTPUT_DIR / f"idealLoad_temperature_{zone}.png"
    plot_comparison(
        measured=meas_al, simulated=sim_al,
        title=f"IdealLoad Temp | {zone}", ylabel=YLABEL,
...
def main():
    ...
    print(f"--- idealLoad_model: Temperature Validation ---")
    ...
    if results:
        ...
        csv_path = OUTPUT_DIR / "idealLoad_temperature_metrics.csv"
