...
parser = argparse.ArgumentParser(description='IdealLoad Model Comparison Run.')
...
# IdealLoad always uses the 'Scaled' strategy (g_val = max(0.3, val)) but accepting
# the argument prevents argparse rejection if an orchestrator passes it uniformly.
parser.add_argument('--eq_sch_type', type=str, default='Scaled', choices=['Scaled', 'Constant', 'Softer'], help='Equipment Schedule Strategy (always Scaled for IdealLoad)')
...
logger = logging.getLogger("sdh_idealload_comparison")
logging.basicConfig(level=logging.INFO)
logger.info(f"Initializing IdealLoad Comparison Run: Mode={args.schedule_mode}, Eq={args.eq_density}")
...
building = model.getBuilding(); building.setName("SDH_Level4_IdealLoad_Comp"); building.setNorthAxis(28)
...
# =========================================================================================
# UNIT 6: LOAD ASSIGNMENT & THERMOSTATS (IDEAL AIR LOADS)
# =========================================================================================
...
visited_zones = set()   # BUG9-pattern: assign thermostat exactly once per zone
for sname in space_map.keys():
    ...
    if zone.handle() not in visited_zones:
        tstat = openstudio.model.ThermostatSetpointDualSetpoint(model); tstat.setHeatingSetpointTemperatureSchedule(sch_20); tstat.setCoolingSetpointTemperatureSchedule(sch_24)
        zone.setThermostatSetpointDualSetpoint(tstat); zone.setUseIdealAirLoads(True)
...
for vn in ["Zone Air Temperature", "Zone Air Relative Humidity", "Zone Air CO2 Concentration", "Zone People Occupant Count", "Zone Ideal Loads Supply Air Total Cooling Energy"]:
...
with open(EXPERIMENT_ROOT / "experiment_config.json", 'w') as f:
    json.dump({"experiment_id": "idealLoad_comp", "latest_run_dir": run_name}, f, indent=4)
