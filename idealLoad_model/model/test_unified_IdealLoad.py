...
# --- PARAMETER INJECTION (For Comparison Parity) ---
parser = argparse.ArgumentParser(description='IdealLoad Model Sensitivity Run.')
...
def create_sch(model, n, p, wv):
    ...
    # Ensure Sizing Period Activity (Parity with Aswani)
    ...
    if f_p.exists() and not args.use_baseline:
        ...
        for d in df['dt'].dt.date.unique():
            ...
            for _, row in day_d.iterrows():
                ...
                o_val = min(1.0, val * 0.6) if val >= 0.35 else 0.0
                
                # Logic: Gains based on eq_sch_type (Parity with Aswani)
                if args.eq_sch_type == 'Scaled':
                    g_val = max(0.3, val) if val >= 0.35 else 0.2
                elif args.eq_sch_type == 'Constant':
                    g_val = 1.0
                elif args.eq_sch_type == 'Softer':
                    g_val = max(0.15, val) if val >= 0.35 else 0.1
                else:
                    g_val = max(0.3, val) if val >= 0.35 else 0.2
            ...
            rs_o.addValue(openstudio.Time(0, 24, 0, 0), min(1.0, last_val * 0.6) if last_val >= 0.35 else 0.0)
            
            if args.eq_sch_type == 'Scaled': lg_val = max(0.3, last_val) if last_val >= 0.35 else 0.2
            elif args.eq_sch_type == 'Constant': lg_val = 1.0
            elif args.eq_sch_type == 'Softer': lg_val = max(0.15, last_val) if last_val >= 0.35 else 0.1
            else: lg_val = max(0.3, last_val) if last_val >= 0.35 else 0.2
...
log_audit(6, "Thermal", "EquipmentDef", "Calibration", "Target EPD for SDH", f"{args.eq_density} W/m2")
...
visited_zones = set() # Tracks zones already given a thermostat to prevent orphaned objects
for sname in space_map.keys():
    ...
    # System Configuration: Assign one Thermostat and enable IdealAirLoads per Zone
    zone = space.thermalZone().get()
    if zone.handle() not in visited_zones:
        tstat = openstudio.model.ThermostatSetpointDualSetpoint(model)
        tstat.setHeatingSetpointTemperatureSchedule(sch_20)
        tstat.setCoolingSetpointTemperatureSchedule(sch_24)
