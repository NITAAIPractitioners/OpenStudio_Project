...
logger = logging.getLogger("sdh_noidealload_model")
logging.basicConfig(level=logging.INFO)
logger.info("Initializing High-Fidelity NoIdealLoad Build (Vault Optimized)...")
...
building.setName("SDH_Level4_NoIdealLoad")
...
for sname, (x, y, w, l, tzn) in space_map.items():
    ...
    # Robust construction assignment for vault compatibility
...
# =========================================================================================
# UNIT 6: LOAD ASSIGNMENT & THERMOSTATS (HVAC OFF)
# =========================================================================================
...
for sname in space_map.keys():
    ...
    log_audit(6, "Thermal", sname, "Assign", "Loads/Vent integrated (HVAC OFF)", "Complete")
...
# BUG10-FIX: Removed "Zone Ideal Loads Supply Air Total Cooling/Heating Energy" from
# output requests — this is a NoIdealLoad (free-floating) model, so no IdealAirLoads
# system exists. EnergyPlus issues "Key not found" warnings and writes empty SQL rows
# for those variables. The comparison script (test_comparison_ASHRAE_NoIdealLoad.py)
# correctly omits them; this file now matches.
output_vars = [
    ...
    ("Zone Lights Electricity Rate",  "*", "TimeStep"),
...
osm_path = run_dir / "SDH_Level4_NoIdealLoad.osm"
...
with open(run_dir / "workflow.osw", 'w') as f:
    json.dump({"seed_file": "SDH_Level4_NoIdealLoad.osm", "weather_file": "Oakland_2013_AMY.epw", "steps": []}, f, indent=4)

os_cli = r"C:\openstudioapplication-1.10.0\bin\openstudio.exe"
cmd = [os_cli, "run", "-w", "workflow.osw"]
logger.info(f"Executing NoIdealLoad simulation in Vault: {run_name}")
...
config = {
    "experiment_id": "noIdealLoad_model", 
...
logger.info(f"NoIdealLoad model complete. Status: {config['last_sim_status']}")
if res.returncode != 0:
