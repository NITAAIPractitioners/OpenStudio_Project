...
parser = argparse.ArgumentParser(description='Aswani Model Comparison Run.')
...
logger = logging.getLogger("sdh_aswani_comparison")
logging.basicConfig(level=logging.INFO)
logger.info(f"Initializing Aswani Comparison Run: Mode={args.schedule_mode}, Eq={args.eq_density}")
...
building = model.getBuilding(); building.setName("SDH_Level4_Aswani_Comp"); building.setNorthAxis(28)
...
sim_ctrl.setDoZoneSizingCalculation(True); sim_ctrl.setDoSystemSizingCalculation(True); sim_ctrl.setDoPlantSizingCalculation(True)
...
def create_sch(model, n, p, wv):
    ...
    s.summerDesignDaySchedule().addValue(openstudio.Time(0, 24, 0, 0), 1.0); s.winterDesignDaySchedule().addValue(openstudio.Time(0, 24, 0, 0), 1.0)
    return s

sch_ashrae = create_sch(model, "ASHRAE_Schedules", [("08:00", 0.1), ("18:00", 1.0), ("24:00", 0.1)], 0.1)
# BUG5-FIX: soft fallback schedules (weekend=0.05) match unified scripts; previously fell back to ASHRAE block (weekend=0.1)
...
if args.schedule_mode == 'ashrae':
    ...
    for sname in space_map.keys():
        ...
        if f_p.exists():
            ...
            s_o.summerDesignDaySchedule().addValue(openstudio.Time(0, 24, 0, 0), 1.0); s_o.winterDesignDaySchedule().addValue(openstudio.Time(0, 24, 0, 0), 1.0)
            s_g.summerDesignDaySchedule().addValue(openstudio.Time(0, 24, 0, 0), 1.0); s_g.winterDesignDaySchedule().addValue(openstudio.Time(0, 24, 0, 0), 1.0)
...
# =========================================================================================
# UNIT 6: HVAC & LOAD ASSIGNMENT
# =========================================================================================
ddy_path = MODEL_SCRIPT_DIR / "USA_CA_Oakland.Intl.AP.724930_TMY3.ddy"
if ddy_path.exists():
    rt = openstudio.energyplus.ReverseTranslator(); ddy_model = rt.loadModel(openstudio.path(str(ddy_path))).get()
    [dd.clone(model) for dd in ddy_model.getDesignDays() if "Oakland Intl AP Ann Htg 99.6% Condns DB" in dd.name().get() or "Oakland Intl AP Ann Clg .4% Condns DB=>MWB" in dd.name().get()]

sch_20 = openstudio.model.ScheduleConstant(model); sch_20.setValue(20.0); sch_24 = openstudio.model.ScheduleConstant(model); sch_24.setValue(24.0)
tstat = openstudio.model.ThermostatSetpointDualSetpoint(model); tstat.setName("Aswani_TStat"); tstat.setHeatingSetpointTemperatureSchedule(sch_20); tstat.setCoolingSetpointTemperatureSchedule(sch_24)
...
air_loop = openstudio.model.AirLoopHVAC(model); air_loop.setName("SDH_VAV_Comparison")
fan = openstudio.model.FanVariableVolume(model); htg_coil = openstudio.model.CoilHeatingElectric(model); clg_coil = openstudio.model.CoilCoolingWater(model)
chw_loop = openstudio.model.PlantLoop(model); chw_pump = openstudio.model.PumpVariableSpeed(model); chiller = openstudio.model.ChillerElectricEIR(model)
chw_pump.addToNode(chw_loop.supplyInletNode()); chiller.addToNode(chw_loop.supplyInletNode()); chw_loop.addDemandBranchForComponent(clg_coil)
chw_sat_sch = openstudio.model.ScheduleConstant(model); chw_sat_sch.setValue(6.7); chw_spm = openstudio.model.SetpointManagerScheduled(model, chw_sat_sch); chw_spm.addToNode(chw_loop.supplyOutletNode())
oa_controller = openstudio.model.ControllerOutdoorAir(model); oa_system = openstudio.model.AirLoopHVACOutdoorAirSystem(model, oa_controller); oa_system.addToNode(air_loop.supplyInletNode())
htg_coil.addToNode(air_loop.supplyInletNode()); clg_coil.addToNode(air_loop.supplyInletNode()); fan.addToNode(air_loop.supplyInletNode())
sat_sch = openstudio.model.ScheduleConstant(model); sat_sch.setValue(13.0); spm = openstudio.model.SetpointManagerScheduled(model, sat_sch); spm.addToNode(air_loop.supplyOutletNode())

visited_zones = set()
for sname in space_map.keys():
    space = model.getSpaceByName(sname).get(); s_o, s_g = space_sch_map[sname]
    peak_cap = 8.0 if "OpenWorkspace" in sname else 1.0
    pp_inst = openstudio.model.PeopleDefinition(model); pp_inst.setName(f"PeopleDef_{sname}"); pp_inst.setNumberofPeople(peak_cap); pp_inst.setSensibleHeatFraction(0.577)  # BUG4-FIX: was missing; all other 5 scripts set 0.577
    ...
    if tz.handle() not in visited_zones:
        ...
        terminal.setConstantMinimumAirFlowFraction(0.15); air_loop.addBranchForZone(tz, terminal)
        sz = openstudio.model.SizingZone(model, tz); sz.setZoneHeatingDesignSupplyAirTemperature(40.0); sz.setZoneCoolingDesignSupplyAirTemperature(13.0); visited_zones.add(tz.handle())

# =========================================================================================
# UNIT 7: OUTPUT & EXECUTE
# =========================================================================================
...
with open(EXPERIMENT_ROOT / "experiment_config.json", 'w') as f:
    json.dump({"experiment_id": "aswani_comp", "latest_run_dir": run_name}, f, indent=4)
