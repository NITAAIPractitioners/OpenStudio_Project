...
# --- Configuration ---
# BUG2-FIX: this script previously only referenced 2 of 3 models, used stale
# hardcoded run IDs, and used key names inconsistent with aggregate_performance_matrix.py.
# Fixed: (1) added Not-Ideal, (2) key names now match aggregator, (3) dynamic latest run.

_HERE          = Path(__file__).parent          # aswani_model/
VALIDATION_DIR = _HERE / "validation"
PROJECT_ROOT   = _HERE.parent

# BUG2-FIX: same find_latest_run() helper used in aggregate_performance_matrix.py
...
# BUG2-FIX: three models defined with canonical names matching aggregate_performance_matrix.py
# BUG2-FIX: each model has its own root so VAL_SQL_PATH points to the correct vault
MODELS = {
...
def run_validations():
    ...
    for model_name, model_root in MODELS.items():
        run_dir = find_latest_run(model_root)   # BUG2-FIX: was hardcoded
        ...
        env["VAL_SQL_PATH"] = str(sql_path)          # BUG2-FIX: per-model SQL path
