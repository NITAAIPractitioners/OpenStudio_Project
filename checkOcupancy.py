...
def process_office_full(office_path):
    def load_sensor(file_name):
        df = pd.read_csv(
            ...
            on_bad_lines='skip',
            engine='python' # helps with skipping bad lines safely
        )
    ...
    # 20-minute shift backward (2 periods in 10-minute resample)
    df_10["co2_lag"] = df_10["CO2"].shift(-2)
    df_10["co2_norm"] = normalize(df_10["co2_lag"])
...
def save_day_plots(df, office_name, date_str, save_folder):
    ...
    plt.plot(day_df.index, day_df["co2_norm"], label="CO2 lag norm")
